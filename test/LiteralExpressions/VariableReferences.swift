...
// RUN: split-file %s %t
// RUN: %target-swift-frontend -typecheck -dump-ast %t/client.swift -I %t/deps -enable-experimental-feature LiteralExpressions | %FileCheck %s
...
@section("mysection") let clangConstantOp7 = 7 + static_const_long
// CHECK-LABEL: (pattern_named type="Int" "clangConstantOp7")
// CHECK: (processed_constant_folded_init=integer_literal_expr implicit type="Int" location={{.*}}client.swift:{{.*}} range=[{{.*}}] value="49"
