...
static void discoverCrossImportOverlayFiles(
    ...
    while (!worklist.empty()) {
      ...
      if (isSwiftDependencyKind(moduleID.Kind)) {
        for (const auto &directSwiftDepID :
          ...
          if (dependencyIDs.count(directSwiftDepID))
            continue;
      ...
      for (const auto &directClangDepID :
        ...
        if (dependencyIDs.count(directClangDepID))
          continue;
  ...
  for (const auto &[_, dependencyIDs] :
       perSourceFileDependencies)
    discoverCrossImportOverlayFilesForModuleSet(dependencyIDs);
}
