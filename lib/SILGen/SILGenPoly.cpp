...
static ManagedValue createThunk(SILGenFunction &SGF,
  ...
  auto *thunk = evaluateOrFatal(
      SGF.SGM.getASTContext().evaluator,
      ReabstractionThunkBodyRequest{ReabstractionThunkBodyDescriptor{
          thunkType, sourceType, toType, dynamicSelfType, globalActorForThunk,
          inputOrigType, inputSubstType, outputOrigType, outputSubstType,
          expectedType, genericEnv, SGF.FunctionDC}});
...
SILFunction *SILGenModule::getOrCreateCustomDerivativeThunkImpl(
...
SILFunction *SILGenModule::getOrCreateCustomDerivativeThunk(
    ...
    SILFunction *customDerivativeFn, const AutoDiffConfig &config,
    AutoDiffDerivativeFunctionKind kind) {
  auto *thunk = getOrCreateCustomDerivativeThunkImpl(
      originalAFD, originalFn, customDerivativeFn, config, kind);
  canonicalizeSynthesizedOnDemand(thunk);
  return thunk;
}
...
ManagedValue SILGenFunction::emitActorIsolationErasureThunk(
  ...
  auto *thunk = evaluateOrFatal(
      SGM.getASTContext().evaluator,
      PreconditionClosureThunkBodyRequest{PreconditionClosureThunkBodyDescriptor{
          thunkType, loweredIsolatedType, expectedType, dynamicSelfType,
          globalActor->getCanonicalType(), isolatedType, nonIsolatedType,
          globalActor, genericEnv, FunctionDC, loc}});
...
SILFunction *ReabstractionThunkBodyRequest::evaluate(
    Evaluator &evaluator, ReabstractionThunkBodyDescriptor d) const {
  auto *SGM = d.thunkType->getASTContext().getActiveSILGenModule();
  assert(SGM && "ReabstractionThunkBodyRequest evaluated outside SILGen scope");
  auto *thunk = SGM->getOrCreateReabstractionThunk(
      d.thunkType, d.fromType, d.toType, d.dynamicSelfType, d.globalActor);
  DEBUG_WITH_TYPE("silgen-requests",
                  silgen_trace::traceIndent()
                      << "-> ReabstractionThunkBodyRequest: "
                      << thunk->getName() << "\n");
  silgen_trace::TraceScope _ts;
  if (thunk->empty()) {
    thunk->setGenericEnvironment(d.genericEnv);
    SILGenFunction thunkSGF(*SGM, *thunk, d.fnDC);
    auto loc = RegularLocation::getAutoGeneratedLocation();
    buildThunkBody(thunkSGF, loc, d.inputOrigType, d.inputSubstType,
                   d.outputOrigType, d.outputSubstType, d.expectedType,
                   d.dynamicSelfType);
    SGM->emitLazyConformancesForFunction(thunk);
  }
  SGM->canonicalizeSynthesizedOnDemand(thunk);
  return thunk;
}

void swift::simple_display(llvm::raw_ostream &out,
                           const ReabstractionThunkBodyDescriptor &) {
  out << "reabstraction thunk body";
}

SourceLoc
swift::extractNearestSourceLoc(const ReabstractionThunkBodyDescriptor &) {
  return SourceLoc();
}

SILFunction *PreconditionClosureThunkBodyRequest::evaluate(
    Evaluator &evaluator, PreconditionClosureThunkBodyDescriptor d) const {
  auto *SGM = d.thunkType->getASTContext().getActiveSILGenModule();
  assert(SGM &&
         "PreconditionClosureThunkBodyRequest evaluated outside SILGen scope");
  auto *thunk = SGM->getOrCreateReabstractionThunk(
      d.thunkType, d.fromType, d.toType, d.dynamicSelfType, d.globalActor);
  DEBUG_WITH_TYPE("silgen-requests",
                  silgen_trace::traceIndent()
  ...
  if (thunk->empty()) {
    thunk->setGenericEnvironment(d.genericEnv);
    SILGenFunction thunkSGF(*SGM, *thunk, d.fnDC);
    // Call-site location (see the descriptor's loc field).
    auto loc = d.loc;
    auto globalActor = d.globalActorForProlog;
    buildThunkBody(
        thunkSGF, loc, AbstractionPattern(d.isolatedType), d.isolatedType,
        AbstractionPattern(d.nonIsolatedType), d.nonIsolatedType,
        d.toType, d.dynamicSelfType,
...
void swift::simple_display(llvm::raw_ostream &out,
                           const PreconditionClosureThunkBodyDescriptor &) {
  out << "actor-isolation-erasure thunk body";
}
...
SILFunction *WithoutActuallyEscapingThunkBodyRequest::evaluate(
    Evaluator &evaluator, WithoutActuallyEscapingThunkBodyDescriptor d) const {
  auto *SGM = d.thunkType->getASTContext().getActiveSILGenModule();
  assert(SGM && "WithoutActuallyEscapingThunkBodyRequest evaluated outside "
                "SILGen scope");
  auto *thunk = SGM->getOrCreateReabstractionThunk(
      d.thunkType, d.noEscapingFnTy, d.escapingFnTy, d.dynamicSelfType,
      CanType());
  DEBUG_WITH_TYPE("silgen-requests",
                  silgen_trace::traceIndent()
                      << "-> WithoutActuallyEscapingThunkBodyRequest: "
  ...
  if (thunk->empty()) {
    ...
    thunk->setGenericEnvironment(d.genericEnv);
    SILGenFunction thunkSGF(*SGM, *thunk, d.fnDC);
    buildWithoutActuallyEscapingThunkBody(thunkSGF, d.dynamicSelfType);
    SGM->emitLazyConformancesForFunction(thunk);
  }
  SGM->canonicalizeSynthesizedOnDemand(thunk);
  return thunk;
...
void swift::simple_display(llvm::raw_ostream &out,
                           const WithoutActuallyEscapingThunkBodyDescriptor &) {
  out << "without-actually-escaping thunk body";
}
