...
static llvm::cl::opt<bool> SILGenOnDemandEmission(
    ...
    llvm::cl::desc("Use request-based on-demand emission instead of "
                   "emitSourceFile (for testing the on-demand SILGen path)"));
...
SILGenModule::emitWitnessForDerivativeAttr(AbstractFunctionDecl *afd,
  ...
  SILDifferentiabilityWitness *lastWitness = nullptr;
  for (auto *origAFD : derivAttr->getOriginalFunctions(getASTContext())) {
    ...
    AutoDiffConfig config(derivAttr->getParameterIndices(), resultIndices,
                          witnessGenSig);
    lastWitness = emitDifferentiabilityWitness(origAFD, origFn,
...
static void emitOnDemandViaRequests(Evaluator &evaluator,
  ...
  while (!worklist.empty()) {
    ...
    // evaluateOrDefault a cyclic @_transparent / @inline(__always) chain at the
    // top level is diagnosed by the evaluator via
    // CanonicalSILFunctionRequest::diagnoseCycle and the outer request returns
    // its default.
    (void)evaluateOrDefault(evaluator,
