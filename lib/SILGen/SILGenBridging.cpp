...
ManagedValue SILGenFunction::emitFuncToBlock(SILLocation loc,
  ...
  auto *thunk = evaluateOrFatal(
      ...
      FuncToBlockThunkBodyRequest{FuncToBlockThunkBodyDescriptor{
          invokeTy, loweredFuncUnsubstTy, loweredBlockTy, funcType, blockType,
          storageTy, useWithoutEscapingVerification, genericEnv, FunctionDC}});
...
SILFunction *BlockToFuncThunkBodyRequest::evaluate(
    Evaluator &evaluator, BlockToFuncThunkBodyDescriptor d) const {
  auto *SGM = d.thunkTy->getASTContext().getActiveSILGenModule();
  assert(SGM && "BlockToFuncThunkBodyRequest evaluated outside SILGen scope");
  auto *thunk = SGM->getOrCreateReabstractionThunk(
      d.thunkTy, d.loweredBlockTy, d.loweredFuncUnsubstTy, CanType(), CanType());
  DEBUG_WITH_TYPE("silgen-requests",
                  silgen_trace::traceIndent()
                      << "-> BlockToFuncThunkBodyRequest: " << thunk->getName()
                      << "\n");
  ...
  if (thunk->empty()) {
    ...
    buildBlockToFuncThunkBody(thunkSGF, loc, d.blockType, d.funcType,
                              d.loweredBlockTy, d.loweredFuncUnsubstTy);
...
void swift::simple_display(llvm::raw_ostream &out,
                           const BlockToFuncThunkBodyDescriptor &) {
  out << "block-to-func thunk body";
}

SourceLoc
swift::extractNearestSourceLoc(const BlockToFuncThunkBodyDescriptor &) {
  return SourceLoc();
}
...
SILGenFunction::emitBlockToFunc(SILLocation loc,
  ...
  auto *thunk = evaluateOrFatal(
      SGM.getASTContext().evaluator,
      BlockToFuncThunkBodyRequest{BlockToFuncThunkBodyDescriptor{
          thunkTy, loweredBlockTy, loweredFuncUnsubstTy, blockType, funcType,
...
SILFunction *FuncToBlockThunkBodyRequest::evaluate(
    Evaluator &evaluator, FuncToBlockThunkBodyDescriptor d) const {
  auto *SGM = d.invokeTy->getASTContext().getActiveSILGenModule();
  assert(SGM && "FuncToBlockThunkBodyRequest evaluated outside SILGen scope");
  auto *thunk = SGM->getOrCreateReabstractionThunk(
      d.invokeTy, d.loweredFuncUnsubstTy, d.loweredBlockTy, CanType(),
      CanType());
  DEBUG_WITH_TYPE("silgen-requests",
                  silgen_trace::traceIndent()
                      << "-> FuncToBlockThunkBodyRequest: " << thunk->getName()
                      << "\n");
  silgen_trace::TraceScope _ts;
  if (thunk->empty()) {
    thunk->setGenericEnvironment(d.genericEnv);
    SILGenFunction thunkSGF(*SGM, *thunk, d.fnDC);
    auto loc = RegularLocation::getAutoGeneratedLocation();
    buildFuncToBlockInvokeBody(thunkSGF, loc, d.funcType, d.blockType,
                               d.loweredFuncUnsubstTy, d.loweredBlockTy,
                               d.storageTy, d.useWithoutEscapingVerification);
    SGM->emitLazyConformancesForFunction(thunk);
  }
  SGM->canonicalizeSynthesizedOnDemand(thunk);
  return thunk;
...
void swift::simple_display(llvm::raw_ostream &out,
                           const FuncToBlockThunkBodyDescriptor &) {
  out << "func-to-block thunk body";
}

SourceLoc
swift::extractNearestSourceLoc(const FuncToBlockThunkBodyDescriptor &) {
  return SourceLoc();
}
