...
// SILGenCleanup performs peephole cleanup of SILGen output before diagnostic
// passes. The pass manager runs function passes in bottom-up call-graph order
// (via BottomUpFunctionOrder), ensuring closures are cleaned up before their
// parent functions.
//
// The needBreakInfiniteLoops / needCompleteLifetimes flags set during SILGen
// are cleared by postEmitFunction, so the pass manager's pre-condition
// (flags must be false before any function pass) is satisfied. This pass
// unconditionally calls breakInfiniteLoops / completeAllLifetimes to handle
// any issues in raw SIL, and clears any flags set by its own CFG modifications
// (e.g. removeUnreachableBlocks) before returning.
