...
static void diagnoseError(const Expr *errorExpr, errorDiagnosis reason,
                          const ASTContext &astContext) {
  DiagnosticEngine &diags = astContext.Diags;
  SourceLoc errorLoc = errorExpr->getLoc();
  switch (reason) {
    case TypeNotSupported:
      diags.diagnose(errorLoc, diag::const_unsupported_binary_operator);
      break;
...
static bool supportedBinaryOperator(const BinaryExpr *binaryExpr) {
...
static std::optional<std::pair<const Expr *, errorDiagnosis>> checkSupportedInConst(const Expr *expr) {
  ...
  while (!expressionsToCheck.empty()) {
    ...
    // Lookthrough IdentityExpr, Tuple, Array, and InjectIntoOptional expressions.
...
static void verifyConstArguments(const CallExpr *callExpr,
  ...
  for (unsigned constantIndex : constArgumentIndices) {
    ...
    if (auto error = checkSupportedInConst(arguments[constantIndex]))
      diagnoseError(error->first, error->second, ctx);
  }
}

void swift::diagnoseInvalidConstExpressions(
    const Expr *expr, const DeclContext *declContext) {
  // verify all arguments to @const parameters
  if (auto *callExpr = dyn_cast<CallExpr>(expr))
    verifyConstArguments(callExpr, declContext->getASTContext());
  
  // verify all @const global initializers
//  if (auto *nonLocalVarDecl = declContext->getNonLocalVarDecl())
//    if (nonLocalVarDecl->isConstValue())
//      if (auto error = checkSupportedInConst(expr))
//        diagnoseError(error->first, error->second, declContext->getASTContext());
  
  // verify all @const local, property initializers
  if (auto *init = dyn_cast<PatternBindingInitializer>(declContext))
    if (auto *binding = init->getBinding())
      if (auto *singleVar = dyn_cast<AbstractStorageDecl>(binding->getSingleVar()))
        if (singleVar->isConstValue())
          if (auto error = checkSupportedInConst(expr))
            diagnoseError(error->first, error->second, declContext->getASTContext());
}
